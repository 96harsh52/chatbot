        ...
        for valid_url in valid_links:
            try:
                ...
                with open(f'logs.log', 'a', encoding='utf-8') as log_file:
                    ...
                    log_file.write(f"[{timestamp}] Successfully extracted text from {valid_url} and updated {website_folder}/{page_name}.txt\n")
                ...
                with open(f'error.log', 'a', encoding='utf-8') as log_file:
                    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    log_file.write(f"[{timestamp}] Failed to fetch the content from the URL: {valid_url}\n")
    except requests.RequestException as e:
